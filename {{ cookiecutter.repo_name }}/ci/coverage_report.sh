...
RPOJ_DIR=$PWD
COVERAGE_REPORT_FILE=$PROJ_DIR/coverage-reports/coverage.xml
...
total_coverage_ratio=$(egrep 'line-rate="0\.[0-9]+"' $COVERAGE_REPORT_FILE | head -n 1 | sed -nE 's/^.+line-rate="0\.([0-9]{2}).+$/\1/p')
echo $total_coverage_ratio

if [[ (-z "$new_coverage_ratio" || $new_coverage_ratio -gt $new_coverage_threshold)  && $total_coverage_ratio -ge $total_coverage_threshold ]]
then
  echo "测试覆盖率很好，请继续保持 ~"
  exit 0
fi
